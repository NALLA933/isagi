...
BANK_CFG = {'int_rate': 0.05, 'loan_int': 0.10, 'max_loan': 100000, 'loan_days': 3, 'penalty': 0.20}
...
async def init_user(uid):
    await user_collection.insert_one({
        'id': uid, 'balance': 0, 'bank': 0, 'user_xp': 0,
        'last_daily': None, 'last_interest': None, 'loan_amount': 0,
        'loan_due_date': None, 'notifications': [], 'permanent_debt': 0
    })
            ...
            async for user in user_collection.find({'loan_amount': {'$gt': 0}, 'loan_due_date': {'$lt': now}}):
                ...
                if bal >= total:
                    await user_collection.update_one({'id': uid}, {
                        '$inc': {'balance': -total},
                        '$set': {'loan_amount': 0, 'loan_due_date': None}
                    })
                    seized.append(f"💰 {total} ɢᴏʟᴅ ғʀᴏᴍ ᴡᴀʟʟᴇᴛ")
                elif funds >= total:
                    await user_collection.update_one({'id': uid}, {
                        '$set': {'balance': 0, 'bank': bank - (total - bal), 'loan_amount': 0, 'loan_due_date': None}
                    })
                    ...
                    if chars:
                        ...
                        for cid in taken:
                            ...
                            chars.remove(cid)
                        await user_collection.update_one({'id': uid}, {
                            '$set': {'characters': chars, 'loan_amount': 0, 'loan_due_date': None}
                        })
                    else:
                        await user_collection.update_one({'id': uid}, {
                            '$set': {'loan_amount': 0, 'loan_due_date': None},
                            '$inc': {'permanent_debt': debt}
                        })
                ...
                await user_collection.update_one({'id': uid}, {
                    '$push': {'notifications': {'type': 'loan_collection', 'message': msg, 'timestamp': now}}
                })
...
async def balance(update: Update, context: CallbackContext):
    ...
    if interest > 0:
        msg += f"\n✨ ɪɴᴛᴇʀᴇꜱᴛ ᴇᴀʀɴᴇᴅ: <code>+{interest}</code> ɢᴏʟᴅ"
    
    msg += "\n\n───────"
    
    btns = [[InlineKeyboardButton("⟲ ʀᴇғʀᴇꜱʜ", callback_data=f"bal_{uid}")],
            [InlineKeyboardButton("🏦 ʙᴀɴᴋ", callback_data=f"bank_{uid}"),
             InlineKeyboardButton("💳 ʟᴏᴀɴ", callback_data=f"loan_{uid}")]]
...
async def deposit(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        f"╭────────────────╮\n│   ᴅᴇᴘᴏꜱɪᴛ ꜱᴜᴄᴄᴇꜱꜱ   │\n╰────────────────╯\n\n"
        f"⟡ ᴅᴇᴘᴏꜱɪᴛᴇᴅ: <code>{amt}</code> ɢᴏʟᴅ\n⟡ ɪɴᴛᴇʀᴇꜱᴛ: <code>5%</code> ᴅᴀɪʟʏ",
        parse_mode="HTML"
    )
...
async def loan_cmd(update: Update, context: CallbackContext):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"⟡ ɪɴᴛᴇʀᴇꜱᴛ: <code>{int(BANK_CFG['loan_int']*100)}%</code>\n"
            f"⟡ ᴅᴜʀᴀᴛɪᴏɴ: <code>{BANK_CFG['loan_days']}</code> ᴅᴀʏꜱ",
            parse_mode="HTML"
        )
        return
    ...
    await user_collection.update_one({'id': uid}, {
        '$inc': {'balance': amt},
        '$set': {'loan_amount': total, 'loan_due_date': due}
    })
    
    await update.message.reply_text(
        f"╭────────────────╮\n│   ✓ ʟᴏᴀɴ ᴀᴘᴘʀᴏᴠᴇᴅ   │\n╰────────────────╯\n\n"
        f"⟡ ʟᴏᴀɴ: <code>{amt}</code> ɢᴏʟᴅ\n⟡ ɪɴᴛᴇʀᴇꜱᴛ: <code>{interest}</code> ɢᴏʟᴅ\n"
        f"⟡ ᴛᴏᴛᴀʟ ʀᴇᴘᴀʏ: <code>{total}</code> ɢᴏʟᴅ\n"
        f"⟡ ᴅᴜᴇ ɪɴ: <code>{BANK_CFG['loan_days']}</code> ᴅᴀʏꜱ\n\n"
        f"⚠️ ᴘᴇɴᴀʟᴛʏ: <code>{int(BANK_CFG['penalty']*100)}%</code> ɪꜰ ᴏᴠᴇʀᴅᴜᴇ",
        parse_mode="HTML"
    )
...
async def repay(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        f"╭────────────────╮\n│   ✓ ʟᴏᴀɴ ʀᴇᴘᴀɪᴅ   │\n╰────────────────╯\n\n"
        f"⟡ ᴘᴀɪᴅ: <code>{loan}</code> ɢᴏʟᴅ\n⟡ ɴᴇᴡ ʙᴀʟᴀɴᴄᴇ: <code>{bal - loan}</code>",
        parse_mode="HTML"
    )
...
async def pay(update: Update, context: CallbackContext):
    ...
    btns = [[InlineKeyboardButton("✓ ᴄᴏɴꜰɪʀᴍ", callback_data=f"pok_{pid}"),
             InlineKeyboardButton("✗ ᴄᴀɴᴄᴇʟ", callback_data=f"pno_{pid}")]]
    
    await update.message.reply_text(
        f"╭────────────────╮\n│   ᴄᴏɴꜰɪʀᴍ ᴘᴀʏᴍᴇɴᴛ   │\n╰────────────────╯\n\n"
        f"⟡ ᴛᴏ: <b>{rec.first_name}</b>\n⟡ ᴀᴍᴏᴜɴᴛ: <code>{amt}</code> ɢᴏʟᴅ\n\n⏳ ᴇxᴘɪʀᴇꜱ ɪɴ 30ꜱ",
